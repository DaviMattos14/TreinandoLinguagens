...
class ContaCorrente:
    ...
    def Saque(self, valor):
        if (valor > self.__saldo and valor > self.__limite) or valor < 0: raise Exception("Valor de saque é maior que o valor em conta!")
        elif valor <= self.__limite or valoe <= self.__limite: self.__saldo -= valor
...
class Main_Window:
    def __init__(self, tk):
        ...
        self.lbStatus = Label(self.fr7, font=('Times New Roman',14,'bold'),text=' ', padx=25)
        self.lbStatus.pack()
    ...
    def Conta(self):
        try:
            ...
            self.lbStatus['text'] = 'Saldo = {:.2f}'.format(float(self.conta.getSaldo()))
    ...
    def Saque(self):
        try:
        if float(self.entradaValor.get()) <= self.conta.getSaldo() or float(self.entradaValor.get()) <= self.conta.getLimite():
    ...
    def Deposito(self):
        try:
            if float(self.entradaValor.get()) > 0:
                ...
                self.lbStatus['text'] = 'Saldo = {:.2f}'.format(float(self.conta.getSaldo()))
            else: self.lbStatus['text'] = 'Valor de depóstio inválido'
        except (ValueError,Exception):
            self.lbStatus['text'] = 'Valores de depósito inválidos'
            self.entradaValor.delete(0,END)
